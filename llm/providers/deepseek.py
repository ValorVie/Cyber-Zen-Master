...
class DeepSeekProvider(LLMProvider):
    ...
    def set_default_model(self, model_id: str) -> None:
        ...
        if model_id not in self._models:
            raise ValueError(f"Model '{model_id}' is not supported by DeepSeek provider")
    ...
    def chat(
        ...
        **kwargs
        ...
        self.get_model(model_id)  # 驗證模型是否支援
    ...
    def embed(
        ...
        **kwargs
        ...
        Args:
            text: 要嵌入的文字或文字列表
            model: 要使用的模型ID，目前DeepSeek API尚未提供嵌入模型
            **kwargs: 額外的參數
            
        Raises:
            NotImplementedError: DeepSeek API尚未提供嵌入功能
        """
        # 目前DeepSeek API尚未提供嵌入功能
        raise NotImplementedError("DeepSeek provider does not support embeddings yet")
